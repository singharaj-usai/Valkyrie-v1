const forumSections = [
    ...
    { id: 'change-log', name: 'Change Log', summary: 'View the change log for Valkyrie.' },
    { id: 'general', name: 'General Discussion', summary: 'Discuss any topic related to Valkyrie.' },
    { id: 'suggestions-and-ideas', name: 'Suggestions and Ideas', summary: 'Share your suggestions and ideas for Valkyrie.' },
    { id: 'media', name: 'Media', summary: 'Share your media with the community.' },
    { id: 'asset-sharing', name: 'Asset Sharing', summary: 'Share your assets with the community.' },
    { id: 'tutorials', name: 'Tutorials', summary: 'Share your tutorials with the community.' },
    ...
    { id: 'rate-my-character', name: 'Rate My Character', summary: 'Rate my character with the community.' },
    { id: 'memes', name: 'Memes', summary: 'Share your memes with the community.' }
];
...
function loadForumSections(activeSection = null) {
    $.ajax({
        ...
        success: function(sections) {
            ...
            appendSection(sectionsList, sections.find(s => s.id === 'announcements'), activeSection, currentPath);
            appendSection(sectionsList, sections.find(s => s.id === 'change-log'), activeSection, currentPath);

            // General header
            sectionsList.append('<h4 class="list-group-item active">General</h4>');            
            appendSection(sectionsList, sections.find(s => s.id === 'general'), activeSection, currentPath);
            appendSection(sectionsList, sections.find(s => s.id === 'suggestions-and-ideas'), activeSection, currentPath);
            appendSection(sectionsList, sections.find(s => s.id === 'media'), activeSection, currentPath);
            appendSection(sectionsList, sections.find(s => s.id === 'asset-sharing'), activeSection, currentPath);
            appendSection(sectionsList, sections.find(s => s.id === 'tutorials'), activeSection, currentPath);
            appendSection(sectionsList, sections.find(s => s.id === 'game-dev'), activeSection, currentPath);

            
            //  Other
            sectionsList.append('<h4 class="list-group-item active">Other</h4>');            
            appendSection(sectionsList, sections.find(s => s.id === 'support'), activeSection, currentPath);
            appendSection(sectionsList, sections.find(s => s.id === 'off-topic'), activeSection, currentPath);
            appendSection(sectionsList, sections.find(s => s.id === 'rate-my-character'), activeSection, currentPath);
            appendSection(sectionsList, sections.find(s => s.id === 'memes'), activeSection, currentPath);
        },
